...
def main(gammaFw,betaFB,alphaRec,iterationNumber,numberEpochs,timeSteps):
    ...
    for iterationIndex in range(0, iterationNumber):
        ...
        pcNet = PCMLP(memory,alphaRec,betaFB,gammaFw,transp=True).to(device)
        ...
        for epoch in range(0, numberEpochs):  
            ...
            path = os.path.join('models',f"PCT_E{epoch}_I{iterationIndex}_G{gammaFw}_B{betaFB}_A{alphaRec}.pth")
    ...
    np.save(os.path.join('accuracies',f"accTrainingCET__G{gammaFw}_B{betaFB}_A{alphaRec}.npy"), resAll)
